...
methods {
    ...
    // Note: WebAuthn.verify and tryDecodeAuth are complex cryptographic operations
    // We can't summarize tryDecodeAuth with NONDET as it returns a struct
    // The verification will be more conservative but still valuable for access control checks
}
