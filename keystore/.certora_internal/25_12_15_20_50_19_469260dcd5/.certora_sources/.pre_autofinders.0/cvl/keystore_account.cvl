...
methods {
    ...
    // Note: We use wildcard _ prefix for external library calls
...
rule refHashInitializationProtection(env e, bytes32 newRefHash) {
    ...
    require existingRefHash != to_bytes32(0);
    ...
    filtered { f -> !f.isView && f.selector != sig:initialize(bytes32).selector }
{
    ...
    require refHashBefore != to_bytes32(0); // Already initialized
...
rule initializeSetsRefHash(env e, bytes32 newRefHash) {
    bytes32 refHashBefore = refHash();
    require refHashBefore == to_bytes32(0); // Not initialized
    require newRefHash != to_bytes32(0);    // Meaningful initialization
...
rule sanityInitializeReachable(env e, bytes32 newRefHash) {
    bytes32 refHashBefore = refHash();
    require refHashBefore == to_bytes32(0);
